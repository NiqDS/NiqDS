...
OKVED for a batch of INNs, keeping the `run(inn_list, as_of_date, spark)`
signature so skill/metrics/runner.py needs no changes.
"""
...
def run(inn_list: list[str], as_of_date: date, spark: Any = None) -> dict[str, Any]:
    return lookup_column(inn_list, column="okved", cast=str)
