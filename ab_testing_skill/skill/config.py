...
class RegistryConfig:
    ...
    involved_inns_path: Path = PACKAGE_ROOT / "sample_data" / "involved_inns.csv"
    duplicates_path: Path = PACKAGE_ROOT / "sample_data" / "inn_duplicates.csv"
    overlap_path: Path = PACKAGE_ROOT / "sample_data" / "metric_overlap.csv"
    metric_requests_path: Path = PACKAGE_ROOT / "sample_data" / "metric_requests.csv"
...
class SplitConfig:
    ...
    # after stratified split, mean(financial metric) between CG/TG must not
    # differ by more than this fraction (relative to the CG mean) or the
    # splitter reshuffles within strata and retries.
    max_relative_imbalance: float = 0.10
